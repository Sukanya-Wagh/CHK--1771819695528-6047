...
@login_required
def smart_fertilizer_ai(request):
    """Advanced AI-powered fertilizer recommendation system"""
    from .smart_fertilizer_ai import SmartFertilizerAI
    from .models import SmartFertilizerAdvice
    ...
    if request.method == 'POST':
        ...
        ai_engine = SmartFertilizerAI()
        
        # Get comprehensive recommendation
        result = ai_engine.analyze_and_recommend(
            crop_type=crop_type,
            farm_area=farm_area,
            area_unit=area_unit,
            soil_type=soil_type,
        ...
        # Save to database
        advice = SmartFertilizerAdvice.objects.create(
            user=request.user,
            crop_type=crop_type,
            farm_area=farm_area,
            area_unit=area_unit,
            ...
            soil_ph=ph,
            nutrient_deficiencies=result['deficiencies'],
            recommended_fertilizers=result['chemical_fertilizers'],
            organic_alternatives=result['organic_alternatives'],
            fertilizer_schedule=result['schedule'],
            total_cost_estimate=result['cost_analysis']['chemical_total'],
            cost_breakdown=result['cost_analysis'],
            expected_yield_improvement=result['yield_prediction']['improvement_percentage'],
            sustainability_score=result['sustainability_score']
        )
        
        # Add result to context
        advice.ai_result = result
    
    recent_advice = SmartFertilizerAdvice.objects.all().order_by('-created_at')[:10]
    
    context = {
        'advice': advice,
        'recent_advice': recent_advice,
        'crop_choices': ['rice', 'wheat', 'cotton', 'corn', 'tomato', 'potato', 'sugarcane', 'soybean'],
    }
    return render(request, 'core/smart_fertilizer_ai.html', context)
